...
SWFUpload.prototype.queueEvent = function (b, c) {
    if (c == undefined) { c = [] } else { if (!(c instanceof Array)) { c = [c] } } var a = this; if (typeof this.settings[b] === "function") { this.eventQueue.push(function () { this.settings[b].apply(this, c) }); setTimeout(function () { a.executeNextEvent() }, 0) } else { if (this.settings[b] !== null) { throw "Event handler " + b + " is unknown or is not a function" } }
};
...
SWFUpload.prototype.unescapeFilePostParams = function (c) {
    var e = /[$]([0-9a-f]{4})/i; var f = {};
    var d; if (c != undefined) {
        for (var a in c.post) {
...
SWFUpload.prototype.flashReady = function () {
    ...
    if (!a) {
        this.debug("Flash called back ready but the flash movie can't be found."); return
    }
    this.cleanUp(a);
        ...
        init: function (options, swfUploadOptions) {
            ...
            return this.each(function () {
                ...
                var settings = $.extend({
                    ...
                    buttonText: 'رەسىملەرنى تاللاڭ',     // The text to use for the browse button
                ...
                if (flashInstalled) {
                    ...
                    var $button = $('<a />', {
                        'id': settings.id + '-button',
                        'class': 'btn btn-danger'//'class': 'uploadify-button ' + settings.buttonClass
                    }).text(settings.buttonText).css({
                        'height': settings.height + 'px',
                        //'line-height': settings.height + 'px',
                        'width': settings.width + 'px'
                    });
